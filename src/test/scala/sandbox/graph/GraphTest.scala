...
class GraphTest extends FlatSpec with MustMatchers {
  ...
  val bigGraph = DirectionalGraph(edges)
  ...
  it should "return None when graph has no edges" in {
    val g = DirectionalGraph[Int](Set(Edge(n3, n1, 1), Edge(n3, n2, 1)))
    shortestPath(g)(n1, n2) must be(None)
  }

  it should "find 1 hop path" in {
    val g = DirectionalGraph(Set(Edge(n1, n2, 1)))
    shortestPath(g)(n1, n2) must be(Some(Path(List(n1,n2), 1)))
  }

  it should "find 2 hop path" in {
    val g = DirectionalGraph(Set(Edge(n1, n2, 1),Edge(n1, n3, 1),Edge(n2, n4, 2),Edge(n3, n4, 1)))
  ...
  it should "find children with lowest depth" in {
    val g = DirectionalGraph(Set(Edge(n1, n2, 1),Edge(n2, n3, 1),Edge(n3, n5, 1),Edge(n1, n4, 2),Edge(n4, n5, 2)))
    bfs(g)(n1,n5) must be(Some(Path(List(n1,n4,n5), 4)))
  }

  behavior of "dfs"
  it should "search depth first" in {
    val g = DirectionalGraph(Set(Edge(n1, n2, 1),Edge(n1, n3, 1),Edge(n3, n4, 1),Edge(n4, n5, 1)))
